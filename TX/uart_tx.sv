...
FSM_Controller #(.DATA_W(DATA_W)) FSMC ( .i_valid(i_valid), .i_clk(i_clk), .i_rst_n(i_rst_n), .i_par_en(i_par_en), .Start_Bit(Start_Bit), .Serial_start(Serial_start),
					.Serial_load(Serial_load), .Stop_Bit(Stop_Bit), .Enable(Enable), .sel(sel), .o_busy(o_busy) );
					
Serializer #(.DATA_W(DATA_W)) Serial ( .i_data(i_data), .i_clk(i_clk), .i_rst_n(i_rst_n), .start(Serial_start), .load(Serial_load), .out(Out1) );

Parity_Bit_Calculator #(.DATA_W(DATA_W)) Parity ( .i_data(i_data), .Enable(Enable), .i_par_odd(i_par_odd), .P_out(Pout) );
